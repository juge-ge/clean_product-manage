...
const fetchReportData = async () => {
  try {
    ...
    schemeData.value = schemeResponse.data
  } catch (error) {
...
### 1.2 生产工艺
- 主要产品：PCB制造
- 生产规模：${enterpriseData.value.annualOutput || '待填写'}万元/年
...
### 2.3 审核结果
- 总体评分：${auditData.value.overall?.totalScore || 0}分
- 审核等级：${auditData.value.overall?.level || '待评估'}
- 审核状态：${auditData.value.overall?.status || '待完成'}
